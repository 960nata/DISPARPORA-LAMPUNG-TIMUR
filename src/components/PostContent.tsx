            ...
            return (
              <div key={block.id} style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: "0.75rem" }}>
                  {visibleSlides.map(s => (
                    <figure key={s.id} style={{ margin: 0, borderRadius: "12px", overflow: "hidden", border: "1px solid var(--border)" }}>
                      <img src={s.src} alt="" style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover", display: "block" }} />
                    </figure>
                  ))}
                </div>
                {numPages > 1 && (
